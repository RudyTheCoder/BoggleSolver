...
1. The dictionary implementation given assumes each word is a lowercase letter 
   ...
   ```c++
   class Node {
    for(int i=0; i < UNICODE_CHARACTERS; i++){
        ptr[i]= nullptr;
    }
    // UNICODE_CHARACTERS will equal however many characters there are in unicode.
   };
   ```
...
4. Why is the prefix tree better than a binary search tree for implementing 
    ...
    Prefix tree will help predetermine whether a word is valid.

5. Suppose you do not use `IsPrefix` in your `SolveBoard` implementation.  
    How would that affect the program?

    We would have to travel down the tree(null) to determine if the word exist. 
    The visited matrix would also be affected because we would not know whether a letter has been visited.
